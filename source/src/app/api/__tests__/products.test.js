...
describe("POST /api/products/route", () => {
  ...
  it("should handle getProductDocument with null response", async () => {
    axios.post.mockResolvedValue({
      data: { data: null },
    });
    const req = createRequest("getProductDocument");
  ...
  it("should handle getDocumentAppDetailById response status = 2004", async () => {
    axios.post.mockResolvedValue({
      status: 204,
      data: { data: { id: 6 } },
    });
  ...
  it("should handle getPolicyholderDocumentsById", async () => {
    axios.post.mockResolvedValue({
      status: 200,
      data: { data: { id: 9 } },
    });
    const req = createRequest("getPolicyholderDocumentsById", { foo: "bar" });
    const res = await POST(req);
    expect(axios.post).toHaveBeenCalledWith(
  ...
  it("should handle getCalTemplate", async () => {
    axios.post.mockResolvedValue({
      status: 200,
      data: { data: { id: 10 } },
    });
    const req = createRequest("getCalTemplate", { foo: "bar" });
    const res = await POST(req);
    expect(axios.post).toHaveBeenCalledWith(
      "http://mock-api/Products/GetDisplayTemplateById",
      { foo: "bar" },
      expect.objectContaining({
        headers: expect.objectContaining({
          Authorization: "Bearer mock-token",
  ...
  it("should handle getCalTemplate response status = 204", async () => {
    axios.post.mockResolvedValue({
      status: 204,
      data: { data: { id: 11 } },
    });
  ...
  it("should handle AddOrUpdateDisplayCalculateTemplate response status = 200", async () => {
    axios.post.mockResolvedValue({
      status: 200,
      data: { data: { id: 11 } },
    });
    const req = createRequest("AddOrUpdateDisplayCalculateTemplate", {
      foo: "bar",
    });
    const res = await POST(req);
    expect(axios.post).toHaveBeenCalledWith(
      "http://mock-api/Products/AddOrUpdateDisplayCalculateTemplate",
      { foo: "bar" },
      expect.objectContaining({
        headers: expect.objectContaining({
          Authorization: "Bearer mock-token",
