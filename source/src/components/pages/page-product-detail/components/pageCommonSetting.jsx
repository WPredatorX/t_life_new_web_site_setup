          ...
          <Grid item xs={7}>
            ...
            >
              <Grid container spacing={2}>
                ...
                <Grid item xs={"auto"}>
                  <Button
                    data-testid="btn-load-policy-doc"
                    disabled={
                      loadPolicy ||
                      ...
                      return (
                        <>
                          <AppAutocomplete
                            ...
                            disablePortal
                            fullWidth
                            required={watch("selectDoc")}
                            disabled={mode === "VIEW" || !watch("selectDoc")}
                            label="เทมเพลตแสดงผลการคำนวณ"
                            onChange={(event, newValue) => {
                              const currentDoc1 = watch("document_Cal") || [];
                              const tempValue = [...currentDoc1].map((item) => {
                                return {
                                  ...item,
                                  id: newValue?.id,
                              ...
                              clearErrors();
                              onChange(newValue);
                              setValue("document_Cal", tempValue);
                            }}
                            {...otherProps}
                            error={Boolean(errors?.calTemplate)}
                            onBeforeOpen={async () =>
                              await handleFetchCalDisplayTemplate(
                                watch("selectDoc")?.document_code
                              )
                            }
                          />
                          <FormHelperText error={errors?.calTemplate}>
                            {errors?.calTemplate?.message}
                          </FormHelperText>
                        </>
                      );
                    }}
                        ...
                        <Grid item xs={4}>
                          {mode !== "VIEW" && (
                            <Grid container spacing={2} justifyContent={"end"}>
                              ...
                              <Grid item xs="auto">
                                <Button
                                  data-testid="btn-remove-all-doc1"
