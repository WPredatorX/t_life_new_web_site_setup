...
const PageProductsDetail = ({
  ...
  const validationSchema = Yup.object().shape({
    ...
    calTemplate: Yup.object()
      .nullable()
      .when("selectDoc", {
        is: (value) => {
          return Boolean(value);
        },
        then: (schema) => schema.required("จำเป็นต้องระบุข้อมูลนี้"),
      }),
  ...
  const handleFetchCalTemplate = async (quoType) => {
    const payload = {
      dis_template_quo_type: quoType,
    };
    const response = await fetch(`/api/products?action=getCalTemplate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    const dataCal = await response.json();
    const mappedData = Array.from(dataCal).map((item, index) => {
      return {
        ...item,
        id: item?.dis_temp_id,
        label: item?.dis_template_name,
        value: item?.dis_template_code,
      };
    });
    return mappedData;
  };
  ...
  const handleFetchProduct = async () => {
    ...
    try {
      ...
      let responsePolicyHolder = await fetch(
        `/api/products?action=getPolicyholderDocumentsById`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      _selectDoc = Array.from(templateList).find(
        (item) =>
      ...
      // #region โหลดข้อมูล เทมเพลตการแสดงผลการคำนวณ
      debugger;
      const calTemplateList = await handleFetchCalTemplate(
        _selectDoc?.document_code
      );
      const calTemplateCode = dataProduct[0]?.dis_template_code;
      const findCalTemplate = Array.from(calTemplateList).find(
        (item) =>
          item?.dis_template_code?.toLowerCase() ===
          calTemplateCode?.toLowerCase()
      );
      // #endregion
      ...
      const prepareReset = {
        ...
        selectDoc: _selectDoc,
        calTemplate: findCalTemplate,
      };
  ...
  const onSubmit = async (data, event) => {
    ...
    try {
      ...
      debugger;
      const calculateDisplayTemplatePayload = {
        product_plan_id: product_plan_id,
        dis_temp_id: watch("calTemplate")?.id,
        create_by: activator,
        update_by: activator,
      };
      const responseCalculateDisplayTemplate = await fetch(
        "/api/products?action=AddOrUpdateDisplayCalculateTemplate",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(calculateDisplayTemplatePayload),
        }
      );
      ...
      if (
        !response.ok ||
        !responseDocument.ok ||
        !responseCalculateDisplayTemplate.ok
      ) {
              ...
              <Grid item xs={12}>
                ...
                {value === 1 && (
                  <PageCommonSetting
                    ...
                    handleFetchCalDisplayTemplate={handleFetchCalTemplate}
                  />
                )}
              </Grid>
            </Grid>
